...
默认使用 deterministic evaluation mode，不调用外部 LLM，保证 CI 和面试演示稳定。项目已通过 provider 层支持可选真实大模型调用，优先接入 DeepSeek OpenAI-compatible API，并保留 OpenAI-compatible 配置空间。真实模型评测只手动运行，API Key 不进入仓库。

## 核心功能

- QA/RAG 批量评测：从 JSONL 数据集运行问答并生成结构化报告。
- 50 条 RAG 样本：覆盖 fact QA、source hit、multi-doc、no-answer、irrelevant、abnormal、keyword distractor 和 refusal error。
- ChromaDB 最小检索链路：基于 `policy_sample.md` 构建本地 in-memory ChromaDB 索引，返回 `content/source/score/rank`，不可用时 fallback。
...
- Prompt A/B 评测：使用 baseline 与 improved RAG prompt 生成两份真实模型报告，并输出 JSON/Markdown compare report 展示指标变化。
- Agent 过程评测：最小工具调用评测链路，检查最终答案、工具选择、检索来源、visible reasoning trace 与超时。
...
`FastAPI`、`Pydantic`、`pytest`、`HTTPX`、`ChromaDB`、`JSONL`、`JSON report`、`ruff`、`uv`

当前形态是轻量级 CLI + JSON/Markdown report 工具链，不是完整 Web 平台。
...
| ChromaDB | `content`、`source`、`score`、`rank` |
...
- 完整 pytest 基线：`189 passed, 2 skipped`
- RAG 样例评测集：`50` 条
- Agent 样例评测集：`10` 条
- 样例通过率：`66%`
- 样例 Badcase：`17` 条
...
uv run python scripts/replay_badcases.py --input-path ../evals/badcases/badcases.jsonl
...
- 当前 RAG 案例使用本地 Markdown policy 样本和最小 ChromaDB 检索，不是生产级向量库检索。
- 当前指标以关键词召回、来源命中和拒答误判规则为主，还没有接完整 LLM-as-a-Judge。
- 下一步可扩展真实向量库 RAG provider，增加模型对比、LLM-as-a-Judge、语义相似度和检索分层指标。
- 当前不接前端，重点是 CLI、JSON/Markdown report 和可回归测试链路；后续产品化时可增加 Web 控制台、趋势看板、数据集版本管理、CI 阈值门禁和线上 Badcase 自动回流。

## 面试口径

可以直接说：MVP 阶段我先用 deterministic mode 把评测、报告、Badcase、ChromaDB 本地检索、Agent 工具调用评测和回归链路做稳定，保证本地和 CI 都可复现；随后通过 provider 层接入 DeepSeek 这类 OpenAI-compatible 大模型，用环境变量切换真实模型评测。这样既能展示真实模型调用能力，又不会让普通 pytest 和 GitHub Actions 依赖外部 API。
...
如果面试官问“为什么没有网页”，可以说：我优先做的是质量评测闭环本身，不是展示型 UI。当前 CLI + JSON/Markdown report 已经能覆盖数据集、真实模型调用、指标计算、Prompt A/B 对比、质量门禁和回归测试。等评测链路稳定后，再把 report 和趋势数据接到 Web Dashboard 会更自然。
