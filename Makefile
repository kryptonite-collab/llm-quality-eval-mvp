...
# === Database ===
db-init:
	cd backend && uv run llm_quality_eval db migrate -m "initial" || true
	cd backend && uv run llm_quality_eval db upgrade
	@echo ""
	@echo "✅ Database initialized!"

db-migrate:
	@read -p "Migration message: " msg; \
	uv run --directory backend llm_quality_eval db migrate -m "$$msg"

db-upgrade:
	uv run --directory backend llm_quality_eval db upgrade

db-downgrade:
	uv run --directory backend llm_quality_eval db downgrade

db-current:
	uv run --directory backend llm_quality_eval db current

db-history:
	uv run --directory backend llm_quality_eval db history

# === Server ===
run:
	uv run --directory backend llm_quality_eval server run --reload

run-prod:
	uv run --directory backend llm_quality_eval server run --host 0.0.0.0 --port 8000

routes:
	uv run --directory backend llm_quality_eval server routes

# === Users ===
create-admin:
	@echo "Creating admin user..."
	uv run --directory backend llm_quality_eval user create-admin

user-create:
	uv run --directory backend llm_quality_eval user create

user-list:
	uv run --directory backend llm_quality_eval user list
...
help:
	@echo ""
	@echo "llm_quality_eval - Available Commands"
	@echo "======================================"
	@echo ""
	...
	@echo ""
	@echo "RAG:"
	@echo "  uv run llm_quality_eval rag-ingest <path> -c <collection>  Ingest files"
	@echo "  uv run llm_quality_eval rag-search <query> -c <collection>  Search"
	@echo "  uv run llm_quality_eval rag-collections                     List collections"
	@echo "  uv run llm_quality_eval rag-sources                         List sync sources"
	@echo "  uv run llm_quality_eval rag-source-add                      Add sync source"
	@echo "  uv run llm_quality_eval rag-source-sync <id>                Trigger sync"
	@echo ""
	@echo "Other:"
